<?php

define('NOMBRE_SERVIDOR', 'localhost');
define('NOMBRE_USUARIO', 'root');
define('PASSWORD', '');
define('NOMBRE_DB', 'flybook');

//Rutas de la web
define("SERVIDOR", "http://localhost/flybook");
define("RUTA_REGISTRO", SERVIDOR . "/registro");
define("RUTA_REGISTRO_CORRECTO", SERVIDOR . "/registro-correcto");
define("RUTA_LOGIN", SERVIDOR . "/login");
define("RUTA_CONTACTO", SERVIDOR . "/contacto");
define("RUTA_PUBLICAR", SERVIDOR . "/publicar");
define("RUTA_LOGOUT", SERVIDOR . "/logout");
define("RUTA_LIBRO", SERVIDOR . "/libro");
define("RUTA_PUBLICACIONES", SERVIDOR . "/publicaciones");
define("RUTA_BUSCAR", SERVIDOR . "/buscar");
define("RUTA_COMPRAS", SERVIDOR . "/compras");

//Recursos
define("RUTA_CSS", SERVIDOR . "/css/");
define("RUTA_CSSe", SERVIDOR . "/CSS/");
define("RUTA_JS", SERVIDOR . "/js/");
define("RUTA_IMG", SERVIDOR . "/images/");
define("RUTA_UPLOADS", SERVIDOR . "/uploads/");